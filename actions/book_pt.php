...
if ($slot_id <= 0) {
    ...
    header('Location: ' . ($_SERVER['HTTP_REFERER'] ?? '../pages/pt_bookings.php'));
    exit;
}

$stmt = $db->prepare(
    'SELECT id, trainer_id, start_time, end_time, is_booked
...
$stmt->execute([$uid, $slot['end_time'], $slot['start_time']]);
if ($stmt->fetch()) {
    set_flash('error', 'You already have a PT booking that overlaps this time.');
    header('Location: ' . ($_SERVER['HTTP_REFERER'] ?? '../pages/pt_bookings.php'));
    exit;
}
...
try {
    ...
    if ($stmt->rowCount() !== 1) {
        throw new RuntimeException('Slot was already booked.');
    }
